...
static void _enable(uint32_t page_dir_phys) {
...
if ((page_dir[dir_index] & PAGE_PRESENT) == 0) {
    ...
    if (new_table_phys == 0) {
        error("NO FREE FRAME FOR PAGE TABLE");
        halt();
    }
    if (new_table_phys >= 0x400000) {
        error("NEW PAGE TABLE NOT IDENTIFY_MAPPED");
        halt();
    }
...
void pager_init(void) {
    ...
    _enable((uint32_t)page_dir);
    print(AS_PAG32, "PAGING ENABLED", l_green);
}
